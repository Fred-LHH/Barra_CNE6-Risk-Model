...
def calc_Beta_HS(close_code: pd.DataFrame, close_index: pd.DataFrame, window: int = 252, half_life: int = 63):
    ...
    args = [(ret.iloc[i:i+window, :].copy()) for i in range(len(ret) - window + 1)]
...
def calc_Daily_std(ret: pd.DataFrame, window: int = 252):
    L = 0.5 ** (1 / 42)
    daily_std = {}
    #### 计算Daily std
    ### 采用EWMA估计股票ret的波动率,半衰期42个交易日
    for i in tqdm(range(len(ret) - window + 1), desc='计算Daily std...'):
        ret_ = ret.iloc[i:i + window]
...
def calc_Cumulative_range(price: pd.DataFrame, window):
